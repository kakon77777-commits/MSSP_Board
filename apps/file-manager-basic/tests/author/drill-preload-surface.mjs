...
const subject = path.join(app, "dist", "preload", "preload.js");
const testFile = path.join(here, "preload-surface.test.mjs");
const run = () => spawnSync(process.execPath, ["--test", testFile], { cwd: app, encoding: "utf8", timeout: 30_000 });
const sha = (bytes) => crypto.createHash("sha256").update(bytes).digest("hex");
const expose = 'electron_1.contextBridge.exposeInMainWorld("fileManager", api);';
const attacks = [
  ["remove the declared registration", expose, ""],
  ["add an undeclared registration", expose, `${expose}\nelectron_1.contextBridge.exposeInMainWorld("unexpected", {});`],
  ["duplicate the registration", expose, `${expose}\n${expose}`],
  ["expose raw ipcRenderer as the API", expose, 'electron_1.contextBridge.exposeInMainWorld("fileManager", electron_1.ipcRenderer);'],
  ["add raw ipcRenderer under a member", "const api = Object.freeze({", "const api = Object.freeze({ ipcRenderer: electron_1.ipcRenderer,"],
];

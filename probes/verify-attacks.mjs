...
const ATTACKS = [
  ...
  { probe: "p17-applicability.mjs", label: "p17 (d) the total skips instead of raising",
    from: 'if (skipped.length) throw new Error(`cannot total across unmeasured units: ${skipped.join(", ")}`);',
    to: "if (skipped.length) return measured.filter((m) => m.applicable).reduce((n, m) => n + m.value, 0);" },
